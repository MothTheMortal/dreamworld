...
class Currency(commands.Cog):
    ...
    @commands.command(
        name="snow",
        aliases=snow_details["aliases"],
        usage=snow_details["usage"],
        description=snow_details["description"],
        signature=snow_details["signature"])
    @commands.has_any_role(*snow_details["required_roles"])
    @commands.cooldown(snow_details["cooldown_rate"], star_details["cooldown_per"])
    async def snow(self, ctx, argument, amount: int, *args: discord.Member):
        ...
        if argument == "add":
            ...
            await ctx.reply(
                f"{amount} {snow} have been added to the respective members.")

            log_embed = self.client.create_embed(
                "Dreamworld Snow Shards Added",
                "Snow Shards have been distributed by a moderator.",
            ...
            log_embed.add_field(name="Amount", value=f"{amount} {snow}",
                                inline=False)

            log_channel = self.client.get_channel(config.channel_ids["logs"])
            return await log_channel.send(embed=log_embed)

        elif argument in ["remove", "subtract"]:

            user_collection.update_many({"_id": {"$in": member_ids}}, {"$inc": {"snow": -1 * amount}})

            await ctx.reply(
                f"{amount} {snow} have been taken from the respective members.")

            log_embed = self.client.create_embed(
                "Dreamworld Snow Shards Removed",
                "Snow Shards have been taken by a moderator.",
                config.embed_info_color
            )
    ...
    @commands.command(
        name="candy",
        aliases=candy2_details["aliases"],
        usage=candy2_details["usage"],
        description=candy2_details["description"],
        signature=candy2_details["signature"])
    @commands.has_any_role(*candy2_details["required_roles"])
    @commands.cooldown(candy2_details["cooldown_rate"], candy2_details["cooldown_per"])
    async def candy(self, ctx, argument, amount: int, *args: discord.Member):
        ...
        if argument == "add":
            user_collection.update_many({"_id": {"$in": member_ids}}, {"$inc": {"candy": amount}})

            await ctx.reply(f"{amount} {candy} have been added to the respective members.")

            log_embed = self.client.create_embed(
                "Dreamworld Candy Added",
                "Candy have been distributed by a moderator.",
                config.embed_info_color
            )

            log_embed.add_field(name="Member(s)", value=", ".join(member_identifications), inline=True)
            log_embed.add_field(name="Moderator", value=f"{ctx.author.name} ({ctx.author.mention})")
            log_embed.add_field(name="Amount", value=f"{amount} {candy}>", inline=False)
            ...
            user_collection.update_many({"_id": {"$in": member_ids}}, {"$inc": {"candy": -1 * amount}})

            await ctx.reply(f"{amount} {candy} have been taken from the respective members.")

            log_embed = self.client.create_embed(
                "Dreamworld Candy Removed",
                "Candy have been taken by a moderator.",
                config.embed_info_color
            )
    ...
    @commands.command(
        name="star",
        aliases=star_details["aliases"],
        usage=star_details["usage"],
        description=star_details["description"],
        signature=star_details["signature"])
    @commands.has_any_role(*star_details["required_roles"])
    @commands.cooldown(star_details["cooldown_rate"], star_details["cooldown_per"])
    async def star(self, ctx, argument, amount: int, *args: discord.Member):
        argument = argument.lower()
        member_ids = list(map(lambda member: member.id, args))
        member_identifications = list(map(lambda member: f"{member.name} ({member.mention})", args))
        user_collection = self.client.get_database_collection("users")
        ...
        if argument == "add":
            user_collection.update_many({"_id": {"$in": member_ids}}, {"$inc": {"star": amount}})

            if len(member_ids) == 1:
                user = await self.client.fetch_member(member_ids[0])
                await ctx.reply(
                    f"{amount} {star} has been added to {user.mention}")
            else:
                await ctx.reply(
                    f"{amount} {star} have been added to the respective members.")

            log_embed = self.client.create_embed(
                "Dreamworld Stars Added",
                "Stars have been distributed by a moderator.",
                config.embed_info_color
            )

            log_embed.add_field(name="Member(s)", value=", ".join(member_identifications), inline=True)
            log_embed.add_field(name="Moderator", value=f"{ctx.author.name} ({ctx.author.mention})")
            log_embed.add_field(name="Amount", value=f"{amount} {star}",
                                inline=False)
            ...
            user_collection.update_many({"_id": {"$in": member_ids}}, {"$inc": {"star": -1 * amount}})

            await ctx.reply(
                f"{amount} {star} have been taken from the respective members.")

            log_embed = self.client.create_embed(
                "Dreamworld Stars Removed",
                "Stars have been taken by a moderator.",
                config.embed_info_color
            )
